...
<template>
  <div class="container">
    ...
    <section v-if="album" class="row py-4 mt-4 album-details justify-content-center align-items-end"
      :style="{ backgroundImage: `url(${album.coverImg})` }">
      <div class="col-10 bg-dark-glass rounded p-3">
        <div class="text-center">
          <h1>{{ album.title }} <span v-if="album.archived"
              class="bg-dark text-danger py-2 px-3 rounded-pill">archived</span></h1>
          <p>{{ album.description }}</p>
        </div>
        <div class="d-flex justify-content-between">
          <div class="d-flex align-items-end">
            <span class="bg-info px-2 py-1 rounded-pill">{{ album.category }}</span>
            <button class="btn btn-danger px-2 py-1 rounded-pill ms-3">archive<i
    ...
    <section class="row my-3">
      ...
      <div class="col-md-8">
        ...
        <button :disabled="!canAddPicture" data-bs-toggle="modal" data-bs-target="#picture-form"
          class="btn btn-primary fab">Add Picture <i class="mdi mdi-camera-plus"></i></button>
